...
namespace AQRPOC.Web
{
    public class PDFConvertor
    {
        ...
        public static void ConvertHTMLToPDFSource(string filePath)
        {
            String fileContents = "";
        ...
        public static string ConvertHTMLToPDF(string headerFile, string bodyFile, string footerFile, float headerHeight, float bodyHeight, float footerHeight)
        {

            string fileContents = "";
            string pdfOutput = System.Configuration.ConfigurationManager.AppSettings["SourcePath"].ToString()+ "PDFOutput_" + DateTime.Now.ToFileTime() + ".pdf";
            ...
            if (!String.IsNullOrEmpty(headerHtml))
            {
                ...
                converter.Header.Height = headerHeight * 0.70f; // convert pixel to point;
        ...
        /// <summary>
        /// Create HTML file runtime from template
        /// </summary>
        ...
        //{
        //    String fileContents = "";
        //    string path = System.Configuration.ConfigurationManager.AppSettings["SourcePath"].ToString(); //@"C:\Users\Home\Desktop\test\";
        //    string templatefile = System.Configuration.ConfigurationManager.AppSettings["TemplateFile"].ToString();
        //    string filePath = path + "template_" + DateTime.Now.ToFileTime() + ".html";
        //    using (StreamReader sr = new StreamReader(path + templatefile))
        //    {
        //        // Read the stream to a string, and write the string to the console.
        //        fileContents = sr.ReadToEnd();
        //    }
        ...
        {
            string fileContents = "";
            string path = System.Configuration.ConfigurationManager.AppSettings["SourcePath"].ToString();
            string templatefile = System.Configuration.ConfigurationManager.AppSettings["TemplateFile"].ToString();
            string filePath = path + "template_" + DateTime.Now.ToFileTime() + ".html";
            using (StreamReader sr = new StreamReader(path + templatefile))
            {
        ...
        public static string CreateTemplateHeader(string htmlString)
        {
            ...
            string templatefile = System.Configuration.ConfigurationManager.AppSettings["TemplateHeaderFile"].ToString();
            using (StreamReader sr = new StreamReader(path + templatefile))
            {
                // Read the stream to a string, and write the string to the console.
                fileContents = sr.ReadToEnd();
        ...
        public static string CreateTemplateFooter(string htmlString)
        {
            ...
            string templatefile = System.Configuration.ConfigurationManager.AppSettings["TemplateFooterFile"].ToString();
            using (StreamReader sr = new StreamReader(path + templatefile))
            {
                // Read the stream to a string, and write the string to the console.
                fileContents = sr.ReadToEnd();
